...
dfm_trimmed <- dfm_trim(dfm, min_docfreq = 0.0025, max_docfreq = 0.99, docfreq_type = "prop")
dfm_trimmed
...
model_10 <- models %>% filter(K == 10) %>% pull(topic_model) %>% .[[1]]
...
T_10 <- labelTopics(model_10, n = 10)
capture.output(T_10, file = "T_10.txt") 
...
findThoughts(model_10, texts = stm[["meta"]][["thread_title"]], n = 4)
...
topicQuality(model_10, documents = stm$documents, main = "model_15")
topicQuality(model_15, documents = stm$documents, main = "model_25")
topicQuality(model_20, documents = stm$documents, main = "model_35")
